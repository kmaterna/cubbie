...
def read_config():
    ...
    orbit_dir=config.get('py-confgi','orbit_dir')
...
def manifest2raw_orig_eof(config_params):
	...
	call(['cp',config_params.orbit_dir+"s1a-aux-cal.xml",'.'],shell=False);
...
def preprocess(config_params):
    ...
    sentinel_utilities.make_data_in(config.params_polarization, config_params.swath, config_params.master);  # makes data.in the first time, with no super_master
...
def write_xml_prep(polarization, swath):
	list_of_xml=sentinel_utilities.get_all_xml_names(polarization,swath);
