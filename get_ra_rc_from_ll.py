...
def get_ra_from_ll(trans_dat, example_grd, lon, lat):
	...
	errorcode = subprocess.call(['gmt','surface','llr',gmtrange,'-bi3f','-I0.005','-T.50','-Gllr.grd','-V'],shell=False);
	if errorcode != 0:
		return [np.nan, np.nan];
	errorcode = subprocess.call(['gmt','surface','lla',gmtrange,'-bi3f','-I0.005','-T.50','-Glla.grd','-V'],shell=False);
	if errorcode != 0:
		return [np.nan, np.nan];
	...
	ra_return = []; az_return = []; 
	[ra, az, z] = np.loadtxt(ra_temp,unpack=True)
	for i in range(len(az)):
		if ra[i]>=range0 and ra[i]<=range1 and az[i]>=az0 and az[i]<=az1:
...
def get_ll_from_ra(trans_dat, ra, az):
	...
	subprocess.call(['rm','gmt.history'],shell=False);
	[lon, lat, z] = np.loadtxt(ll_temp,unpack=True);
	lon=lon[0];
	lat=lat[0];
	return [lon, lat];
