...
def before_after_images(before_phase, after_phase, outfilename):
    ...
    plt.subplot(1, 2, 1);
    plt.imshow(before_phase, cmap='viridis', vmin=np.min(before_phase), vmax=np.max(after_phase));
    plt.title('Before Correction');
    plt.subplot(1, 2, 2);
    plt.imshow(after_phase, cmap='viridis', vmin=np.min(before_phase), vmax=np.max(after_phase));
