...
# Criteria for Reference Pixel:
# 1. High coherence in all interferograms- will give warning if you don't have a pixel with perfect coherence.
...
def main_function(file_dir):
    ...
    [_, _, data_all, _, _] = inputs(filenames);
    ...
    outputs(rowref, colref, range_bounds, azimuth_bounds, number_of_datas);
...
def configure(file_dir):
    ...
    if len(file_names) == 0:
        print("Error! No files matching search pattern with " + file_of_interest);
        sys.exit(1);
...
def analyze_coherent_number(zdata):
    ...
    print("Number of pixels equal to max_possible: %d out of %d ( %f %%)" %
          (best_pixels, rowdim * coldim, 100.0 * best_pixels / (rowdim * coldim)));
...
# -------------- OUTPUTS -------------- #
def outputs(rowref, colref, range_bounds, azimuth_bounds, number_of_datas):
    ...
    plt.plot([range_bounds[0], range_bounds[1], range_bounds[1], range_bounds[0], range_bounds[0]],
             [azimuth_bounds[0], azimuth_bounds[0], azimuth_bounds[1], azimuth_bounds[1], azimuth_bounds[0]], color='k')
