...
generate an expected field of InSAR velocities. 
...
param_collection = collections.namedtuple("param_collection",['input_file','ca_file','or_file','type_of_interp','ascending_flight_angle','descending_flight_angle','incidence_angle','xbounds','ybounds','point1', 'point2', 'reference_point']);
...
def do_interpolation():
	...
	[vel_tuple,ca_border,or_border] = inputs(my_param_collection.input_file, my_param_collection.or_file, my_param_collection.ca_file);
...
def compute(vel_tuple,my_param_collection, ca_border, or_border):
	...
	# We remove the interpolated points outside of CA and OR because they tend to explode. 
	...
	ascending_LOS_array=los_projection_math.project_ENU_to_LOS(new_east,new_north,new_vertical,my_param_collection.ascending_flight_angle,my_param_collection.incidence_angle);
	descending_LOS_array=los_projection_math.project_ENU_to_LOS(new_east,new_north,new_vertical,my_param_collection.descending_flight_angle,my_param_collection.incidence_angle);
	...
	ascending_LOS_reference=los_projection_math.project_ENU_to_LOS(velref_e,velref_n,0,my_param_collection.ascending_flight_angle,my_param_collection.incidence_angle)[0];
	descending_LOS_reference=los_projection_math.project_ENU_to_LOS(velref_e,velref_n,0,my_param_collection.descending_flight_angle,my_param_collection.incidence_angle)[0];
	...
	ascending1=los_projection_math.project_ENU_to_LOS(e_def1,n_def1,0.0,my_param_collection.ascending_flight_angle,my_param_collection.incidence_angle);
	ascending2=los_projection_math.project_ENU_to_LOS(e_def2,n_def2,0.0,my_param_collection.ascending_flight_angle,my_param_collection.incidence_angle);
	descending1=los_projection_math.project_ENU_to_LOS(e_def1,n_def1,0.0,my_param_collection.descending_flight_angle,my_param_collection.incidence_angle);
	descending2=los_projection_math.project_ENU_to_LOS(e_def2,n_def2,0.0,my_param_collection.descending_flight_angle,my_param_collection.incidence_angle);
...
def outputs(vel_tuple, my_param_collection, ca_border, or_border, my_interpolated_fields, asc_ref, des_ref):
	...
	my_plot_formatting(axarr[0],my_param_collection,ca_border,or_border,"Ascending GPS LOS Velocity (interp)",vel_tuple);
	...
	my_plot_formatting(axarr[1],my_param_collection,ca_border,or_border,"Descending GPS LOS Velocity (interp)",vel_tuple);
