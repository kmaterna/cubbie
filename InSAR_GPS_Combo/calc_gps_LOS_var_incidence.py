...
def top_level_driver(config_params):
	...
	outputs(gps_velfield, LOS_velfield, outfile);
	return;
...
def compute(gps_velfield, reference_gps, xarray, yarray, lkv_east, lkv_north, lkv_up):
	...
	for i in range(len(gps_velfield.elon)):
		lkv_e, lkv_n, lkv_u = get_lookvectors_by_nearest_grid(xarray, yarray, lkv_east, lkv_north, lkv_up, gps_velfield.elon[i], gps_velfield.nlat[i]);
		[flight_angle_i, look_angle_i] = los_projection_math.look_vector2flight_incidence_angles(lkv_e, lkv_n, lkv_u);
		LOS_array_i=los_projection_math.simple_project_ENU_to_LOS(gps_velfield.e[i],gps_velfield.n[i],gps_velfield.u[i], flight_angle_i, look_angle_i);
		LOS_array.append(LOS_array_i);

	# Take the reference point and transform its velocity into LOS. 
	velref_e, velref_n, velref_u, reflon, reflat = los_projection_math.get_point_enu_veltuple(gps_velfield, reference_point_name=reference_gps);
	lkv_e_ref, lkv_n_ref, lkv_u_ref = get_lookvectors_by_nearest_grid(xarray, yarray, lkv_east, lkv_north, lkv_up, reflon, reflat);
	[flight_angle_ref, look_angle_ref] = los_projection_math.look_vector2flight_incidence_angles(lkv_e_ref, lkv_n_ref, lkv_u_ref);
	LOS_reference = los_projection_math.simple_project_ENU_to_LOS(velref_e,velref_n,velref_u,flight_angle_ref,look_angle_ref);

	los_tuple=Velfield(name=gps_velfield.name, nlat=gps_velfield.nlat, elon=gps_velfield.elon, e=LOS_array-LOS_reference, n=0*LOS_array, u=0*LOS_array, 
		sn=gps_velfield.sn, se=gps_velfield.se, su=gps_velfield.su, first_epoch=gps_velfield.first_epoch, last_epoch=gps_velfield.last_epoch);

	return [los_tuple];


def closest_index(lst, K): 
	# Given list lst, and target K, which list is closest? 
	# Can return lst[min(range(len(lst)), key = lambda i: abs(lst[i]-K))]  if you want the actual lon value
...
def get_lookvectors_by_nearest_grid(xarray, yarray, lkv_east, lkv_north, lkv_up, target_lon, target_lat):
	...
	xi = closest_index(xarray, target_lon);
	yi = closest_index(yarray, target_lat);
	lkv_e, lkv_n, lkv_u = lkv_east[yi,xi], lkv_north[yi,xi], lkv_up[yi,xi];
	return [lkv_e, lkv_n, lkv_u];


# ------------------- OUTPUTS ------------- # 
def outputs(gps_velfield, LOS_velfield, outfile):
