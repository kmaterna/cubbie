...
def compute(gps_velfield, reference_gps, xarray, yarray, lkv_east, lkv_north, lkv_up):
	...
	for i in range(len(gps_velfield.elon)):
		...
		LOS_array_i=los_projection_tools.simple_project_ENU_to_LOS(gps_velfield.e[i],gps_velfield.n[i],gps_velfield.u[i], flight_angle_i, look_angle_i);
	...
	[flight_angle_ref, look_angle_ref] = los_projection_tools.look_vector2flight_incidence_angles(lkv_e_ref, lkv_n_ref, lkv_u_ref);
	LOS_reference = los_projection_tools.simple_project_ENU_to_LOS(velref_e,velref_n,velref_u,flight_angle_ref,look_angle_ref);

	los_tuple=los_projection_tools.Velfield(name=gps_velfield.name, nlat=gps_velfield.nlat, elon=gps_velfield.elon, e=LOS_array-LOS_reference, n=0*LOS_array, u=0*LOS_array, 
