...
def do_interpolation():
    ...
    [vel_tuple, ca_border, or_border] = inputs(my_param_collection.input_file, my_param_collection.bounds,
                                               my_param_collection.or_file, my_param_collection.ca_file);
    los_ascending_velfield, los_descending_velfield, eastnorthfield = compute(vel_tuple, my_param_collection, ca_border,
                                                                              or_border);
    outputs(vel_tuple, my_param_collection, ca_border, or_border, los_ascending_velfield, los_descending_velfield,
            eastnorthfield);
...
def configure():
    ...
    my_param_collection = param_collection(input_file=gps_input_file, ca_file=ca_file, or_file=or_file,
                                           ...
                                           bounds=bounds, point1=gradient_point1, point2=gradient_point2,
...
def compute(vel_tuple, my_param_collection, ca_border, or_border):
    ...
    [x_for_interp, y_for_interp] = get_interpolation_points_within_grid(my_param_collection.bounds, 0.08,
                                                                        [ca_border, or_border]);
    ...
    for i in range(len(x_for_interp)):
        new_east.append(
            f_east(x_for_interp[i], y_for_interp[i]))  # only want to give the functions one point at a time.
    ...
    velref_e, velref_n, velref_u = los_projection_tools.get_point_enu_interp(my_param_collection.reference_point,
                                                                             f_east, f_north);
    ...
    ascending_LOS_array = los_projection_tools.simple_project_ENU_to_LOS(new_east, new_north, new_vertical,
                                                                         my_param_collection.ascending_flight_angle,
                                                                         my_param_collection.incidence_angle);
    ascending_LOS_reference = los_projection_tools.simple_project_ENU_to_LOS(velref_e, velref_n, velref_u,
                                                                             my_param_collection.ascending_flight_angle,
                                                                             my_param_collection.incidence_angle)[0];
    los_ascending_velfield = los_projection_tools.Velfield(name=[], elon=x_for_interp, nlat=y_for_interp,
                                                           e=ascending_LOS_array - ascending_LOS_reference,
                                                           n=0 * ascending_LOS_array, u=0 * ascending_LOS_array, se=[],
                                                           sn=[], su=[], first_epoch=[], last_epoch=[]);

    # Descending
    descending_LOS_array = los_projection_tools.simple_project_ENU_to_LOS(new_east, new_north, new_vertical,
                                                                          my_param_collection.descending_flight_angle,
                                                                          my_param_collection.incidence_angle);
    descending_LOS_reference = los_projection_tools.simple_project_ENU_to_LOS(velref_e, velref_n, velref_u,
                                                                              my_param_collection.descending_flight_angle,
                                                                              my_param_collection.incidence_angle)[0];
    los_descending_velfield = los_projection_tools.Velfield(name=[], elon=x_for_interp, nlat=y_for_interp,
                                                            e=descending_LOS_array - descending_LOS_reference,
                                                            n=0 * descending_LOS_array, u=0 * descending_LOS_array,
                                                            se=[], sn=[], su=[], first_epoch=[], last_epoch=[]);
    ...
    eastnorthfield = los_projection_tools.Velfield(name=[], elon=x_for_interp, nlat=y_for_interp, e=new_east,
                                                   n=new_north, u=0 * descending_LOS_array, se=[], sn=[], su=[],
    ...
    ascending1 = los_projection_tools.simple_project_ENU_to_LOS(e_def1, n_def1, u_def1,
                                                                my_param_collection.ascending_flight_angle,
                                                                my_param_collection.incidence_angle);
    ascending2 = los_projection_tools.simple_project_ENU_to_LOS(e_def2, n_def2, u_def2,
                                                                my_param_collection.ascending_flight_angle,
                                                                my_param_collection.incidence_angle);
    descending1 = los_projection_tools.simple_project_ENU_to_LOS(e_def1, n_def1, u_def1,
                                                                 my_param_collection.descending_flight_angle,
                                                                 my_param_collection.incidence_angle);
    descending2 = los_projection_tools.simple_project_ENU_to_LOS(e_def2, n_def2, u_def2,
                                                                 my_param_collection.descending_flight_angle,
                                                                 my_param_collection.incidence_angle);
    evaluate_gradients(my_param_collection.point1, my_param_collection.point2, ascending1, ascending2, descending1,
                       descending2);
...
# ---------- PLOTTING OUTPUTS --------------- # 
def outputs(vel_tuple, my_param_collection, ca_border, or_border, los_ascending_velfield, los_descending_velfield,
            eastnorthfield):
    ...
    axarr[0].quiver(quiver_point[0], quiver_point[1],
                    np.cos(np.deg2rad(90 - my_param_collection.ascending_flight_angle)),
                    np.sin(np.deg2rad(90 - my_param_collection.ascending_flight_angle)), scale=10);
    axarr[0].quiver(quiver_point[0], quiver_point[1],
                    np.sin(np.deg2rad(90 - my_param_collection.ascending_flight_angle)),
                    -np.cos(np.deg2rad(90 - my_param_collection.ascending_flight_angle)), scale=10, color='red');
    axarr[0].text(quiver_point[0] + 0.05, quiver_point[1], "LOS", color='red', ha='right', va='top');
    axarr[0].plot(my_param_collection.point1[0], my_param_collection.point1[1], marker='s', color='black',
                  markersize=5);
    axarr[0].plot(my_param_collection.point2[0], my_param_collection.point2[1], marker='s', color='black',
                  markersize=5);
    ...
    axarr[1].quiver(quiver_point[0], quiver_point[1],
                    np.cos(np.deg2rad(90 - my_param_collection.descending_flight_angle)),
                    np.sin(np.deg2rad(90 - my_param_collection.descending_flight_angle)), scale=10);
    axarr[1].quiver(quiver_point[0], quiver_point[1],
                    np.sin(np.deg2rad(90 - my_param_collection.descending_flight_angle)),
                    -np.cos(np.deg2rad(90 - my_param_collection.descending_flight_angle)), scale=10, color='red');
    axarr[1].text(quiver_point[0] + 0.05, quiver_point[1], "LOS", color='red', ha='left', va='bottom');
    axarr[1].quiver(narray_x, narray_y, narray_east, narray_north, color='white', scale=500.0);
    axarr[1].plot(my_param_collection.point1[0], my_param_collection.point1[1], marker='s', color='black',
                  markersize=5);
    axarr[1].plot(my_param_collection.point2[0], my_param_collection.point2[1], marker='s', color='black',
                  markersize=5);
